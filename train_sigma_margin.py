...
def find_sigma_margin(best_data, worst_data, detection_threshold):
    ...
    for sigma in sigma_thresh:
        best_tp=len([a[0,0] for a in best_data if a[0,0]>=(detection_threshold+sigma) if a[0,1]==1])
        best_fn=len([a[0,0] for a in best_data if a[0,0]<(detection_threshold+sigma) if a[0,1]==1])
        best_fp=len([a[0,0] for a in best_data if a[0,0]>=(detection_threshold+sigma) if a[0,1]==0])
        best_tn=len([a[0,0] for a in best_data if a[0,0]<(detection_threshold+sigma) if a[0,1]==0])
        worst_tp=len([a[0,0] for a in worst_data if a[0,0]>=(detection_threshold+sigma) if a[0,1]==1])
        worst_fn=len([a[0,0] for a in worst_data if a[0,0]<(detection_threshold+sigma) if a[0,1]==1])
        worst_fp=len([a[0,0] for a in worst_data if a[0,0]>=(detection_threshold+sigma) if a[0,1]==0])
        worst_tn=len([a[0,0] for a in worst_data if a[0,0]<(detection_threshold+sigma) if a[0,1]==0])
