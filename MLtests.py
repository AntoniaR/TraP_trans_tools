...
def learning_curve(anomaly,logistic,transSrc,data,full_data,lda,options,stable,variable,precis_thresh,recall_thresh):
    ...
    if anomaly:
        if not os.path.exists("AD_learn_data.csv"):
            rangeNums=[int(10.**a) for a in np.arange(0.08,4.01,4./50.)]
            train=np.array(train)
            valid=np.array(valid)
            error_train, error_valid = train_anomaly_detect.learning_curve(stable,variable,train, valid, precis_thresh, recall_thresh, rangeNums)
            tmp = [[rangeNums[x], error_train[x], error_valid[x]] for x in range(len(rangeNums))]
            generic_tools.write_test_data("AD_learn_data.csv",tmp)
        else:
            tmp = generic_tools.extract_data("AD_learn_data.csv")
            rangeNums = [int(x[0]) for x in tmp]
            error_train = [float(x[1]) for x in tmp]
            error_valid = [float(x[2]) for x in tmp]      
        plotting_tools.plotLC(rangeNums,error_train, error_valid, "AD_learning", True, True, "Number")        

    if logistic:
        if not os.path.exists("LR_learn_data.csv"):
            ...
            generic_tools.write_test_data("LR_learn_data.csv",tmp)
        else:
            tmp = generic_tools.extract_data("LR_learn_data.csv")
            rangeNums = [int(x[0]) for x in tmp]
            error_train = [float(x[1]) for x in tmp]
            error_valid = [float(x[2]) for x in tmp]      
        plotting_tools.plotLC(range(len(error_train)),error_train, error_valid, "LR_learning", True, True, "Number")

    if transSrc:
        if not os.path.exists("TransBest_learn_data.csv"):
            ...
            best=best_data+best_sim
            worst=worst_data+worst_sim
            ...
            generic_tools.write_test_data("TransBest_learn_data.csv",tmp)
            tmp = [[x, worst_error_train[x], worst_error_valid[x]] for x in range(len(worst_error_train))]
            generic_tools.write_test_data("TransWorst_learn_data.csv",tmp)
        else:
            tmp = generic_tools.extract_data("TransBest_learn_data.csv")
            rangeNums = [int(x[0]) for x in tmp]
            best_error_train = [float(x[1]) for x in tmp]
            best_error_valid = [float(x[2]) for x in tmp]      
            tmp = generic_tools.extract_data("TransWorst_learn_data.csv")
            rangeNums = [int(x[0]) for x in tmp]
            worst_error_train = [float(x[1]) for x in tmp]
            worst_error_valid = [float(x[2]) for x in tmp]      

        plotting_tools.plotLC(range(len(best_error_train)),best_error_train, best_error_valid, "transBest_learning", True, True, "Number")
        plotting_tools.plotLC(range(len(worst_error_train)),worst_error_train, worst_error_valid, "transWorst_learning", True, True, "Number")
    return


def lambda_curve(data,lda,options):
    # shuffle up the transient and stable data
    shuffled = np.matrix(generic_tools.shuffle_datasets(data))
    ...
    if not os.path.exists("LR_lambda_data.csv"):
        ...
        generic_tools.write_test_data("LR_lambda_data.csv",tmp)
    else:
        tmp = generic_tools.extract_data("LR_lambda_data.csv")
...
def repeat_curve(anomaly,logistic,transSrc,data,full_data,lda,options,stable,variable,precis_thresh,recall_thresh):
    ...
    for counter in range(1000):
        ...
        if anomaly:
            if not os.path.exists("AD_repeat_data.csv"):
    ...
    if anomaly:
        if not os.path.exists("AD_repeat_data.csv"):
            ...
            generic_tools.write_test_data("AD_repeat_data.csv",tmp)
        else:
            tmp = generic_tools.extract_data("AD_repeat_data.csv")
            randomChoice = [int(x[0]) for x in tmp]
            error_train_AD = [float(x[1]) for x in tmp]
            error_valid_AD = [float(x[2]) for x in tmp] 
        plotting_tools.plotLC(randomChoice, error_train_AD, error_valid_AD, "AD_repeat", False, True, "Trial number")
    if logistic:
        if not os.path.exists("LR_repeat_data.csv"):
            tmp = [[x, error_train_LR[x], error_valid_LR[x]] for x in range(len(error_train_LR))]
            generic_tools.write_test_data("LR_repeat_data.csv",tmp)
        else:
            tmp = generic_tools.extract_data("LR_repeat_data.csv")
            randomChoice = [int(x[0]) for x in tmp]
            error_train_LR = [float(x[1]) for x in tmp]
            error_valid_LR = [float(x[2]) for x in tmp] 
        plotting_tools.plotLC(range(len(error_train_LR)), error_train_LR, error_valid_LR, "LR_repeat", False, True, "Trial number")
    if transSrc:
        if not os.path.exists("TransBest_repeat_data.csv"):
            tmp = [[x, error_train_TBest[x], error_valid_TBest[x]] for x in range(len(error_train_TBest))]
            generic_tools.write_test_data("TransBest_repeat_data.csv",tmp)
            tmp = [[x, error_train_TWorst[x], error_valid_TWorst[x]] for x in range(len(error_train_TWorst))]
            generic_tools.write_test_data("TransWorst_repeat_data.csv",tmp)
        else:
            tmp = generic_tools.extract_data("TransBest_repeat_data.csv")
            randomChoice = [int(x[0]) for x in tmp]
            error_train_TBest = [float(x[1]) for x in tmp]
            error_valid_TBest = [float(x[2]) for x in tmp] 
            tmp = generic_tools.extract_data("TransWorst_repeat_data.csv")
            randomChoice = [int(x[0]) for x in tmp]
            error_train_TWorst = [float(x[1]) for x in tmp]
            error_valid_TWorst = [float(x[2]) for x in tmp] 
        plotting_tools.plotLC(range(len(error_train_TBest)), error_train_TBest, error_valid_TBest, "transBest_repeat", False, True, "Trial number")
        plotting_tools.plotLC(range(len(error_train_TWorst)), error_train_TWorst, error_valid_TWorst, "transWorst_repeat", False, True, "Trial number")
    return
